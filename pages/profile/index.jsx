...
export default function Profile() {
  ...
  const { data: dataPost } = useQueries({
    queryKey: ["posts", "me"],
  ...
  const confirmDelete = async () => {
    try {
      const response = await fetch(
        `https://paace-f178cafcae7b.nevacloud.io/api/post/delete/${selectedPostId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${Cookies.get("user_token")}`,
            ...
            <div key={post.id} className="max-w-4xl mx-auto border rounded-2xl py-4 px-4 bg-white">
              <div className="flex items-center mb-4">
                ...
                <div className="ml-auto">
                  <button
                    className="mr-1 px-4 py-1 bg-blue-500 text-gray-100 border border-gray-300 hover:bg-blue-400 rounded-2xl"
                    onClick={() => openEditPostModal(post.id)}
                  >Edit
                  </button>
                  <button
                    className="p-2 px-4 py-1 bg-red-500 text-gray-100 border border-gray-300 hover:bg-red-400 rounded-2xl"
